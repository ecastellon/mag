...
setGeneric("ltxhtml",
           function(x,fltx=character(0),tipo="latex",dec=numeric(0),
...
###
setMethod("existe", "data.frame",
          function(x){
            FALSE
          }
          )
#
setMethod("descripcion","data.frame",
          function(x){
            return("ninguna")
          }
          )
...
#
setMethod("guardar", "data.frame",
          function(x, name, file){
            if(missing(file)) stop("file?")
            if(missing(name)) name <- deparse(substitute(x))
            env <- new.env(hash=FALSE)
            if(file.exists(file)) load(file, envir=env)
...
setMethod("datos", "data.frame",
          function(x, name, file){
            ...
            if(file.exists(file)){
                ...
                cual <- switch(class(name),
                           character = match(name, dfs),
                           numeric = ifelse(name > 0 && name <= length(dfs),
                                            name, NA), NA)
                d <- ifelse(is.na(cual), NULL,
                            get(dfs[cual], envir=env))
                if(is.null(d)) print(paste("no existe", name,"en", file))
                rm(env)
            } else{
...
#tabla latex ntab del data.frame x
#dec:decimales en la columna
#ali:alineacion de las columnas
#nomr:nombre de las hileras de x que definen la columna
#  de identificacion
#clider:columna que determina el orden; 0 orden original
#  >0 orden descendente; <0 ascendente
#glast:cuando orden descendente: la mas grande de ultimo?
#  por aquello de poner totales en la ultima hilera
#---algoritmo ordenar filas de tabla---2010-02-17
#no siempre indicador de país es el más grande (promedios)
#se modifica el algoritmo para considerar esto
#ultimafila es parametro que indica la fila que deberá
#quedar en la misma posición después de ordenar por clider
#ultimafila=0 se ordenan todas las filas y controla glast
#---
#OJO:convertir a metodo de data.frame
#pp(postproceso):adaptaciones propias usando threeparttable
#**ToDo**
#-validacion del nombre de archivo
setMethod("ltxhtml","data.frame",
         function(x,fltx=character(0),tipo="latex",
                  dec=numeric(0),ali=character(0),
                  nomr=character(0),clider=0,glast=1,
                  pp=character(0),ultimafila=0)
          {
            ...
            xtb<-xtable(x)
            #falta mas controles a dec y ali
            #ver los metodos en xtable
            if(length(dec)==0)
              dec <- rep(0,ncol(x)+1)
            digits(xtb)<-dec
            if(length(ali)==0)
            ...
            #version de xtable devuelve string
            #pero si no hay archivo imprime en pantalla
            if(length(fltx)==0) fltx <- ""
            guardar <- fltx!=""
            if(!guardar)
              fltx <- tempfile()
            ...
            if(tipo=="latex"&&length(pp)>0){
              z <- sapply(strsplit(sT,"\n"),
                          ...
                          USE.NAMES=FALSE)
              sT<-z[1:2]
              sT<-c(sT,"%-- modificaciones(E.Castellon)",
                    paste("%--",pp["descripcion"]))
              bL <- (pp["landscape"]=="si")
              if(bL) sT <- c(sT,"\\begin{landscape}")
              ...
              if(pp["formato"]==""){
                ...
                cT <- paste("\\begin{tabular}{",pp["formato"],"}",sep="")
              sT <- c(sT,cT,"\\toprule")
              ...
              if(pp["notas"]!="")
                sT <- c(sT,"\\begin{tablenotes}",pp["notas"],
                        "\\end{tablenotes}")
              if(bFs)
                sT <- c(sT,"}")
              sT <- c(sT,"\\end{threeparttable}","\\end{table}")
              if(bL) sT <- c(sT,"\\end{landscape}")
              if(guardar)
