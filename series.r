...
#
  ...
  minx=min(data,L)
  maxx=max(data,U)
  ts.plot(data,fore$pred,col=1:2, ylim=c(minx,maxx), ylab=deparse(substitute(xdata)))
...
#ejemplo gráfica desviaciones mensuales
#gHylleberg
afin <- end(s.mims)[1]
...
a2 <- rep(start(s.mims)[1]:afin,rep(12,anos))[1:length(s.mims)]
u <- w$time.series#w resultado de STL
