...
anova1ylc <- anova_lrt(anova1ylc,fit.f1,fit.f1ylc,"YR:LC:CLT")
...
#trait count, min, max, mean, sum, median, var, sd over location

mean_l <- tempa2 %>%
  group_by (LC ) %>%
  summarize (count = n(),
             min = min(Trait,na.rm=FALSE),
             max = max(Trait,na.rm=FALSE), 
             mean = mean(Trait,na.rm=FALSE),
             sum = sum(Trait,na.rm=FALSE),
             median = median(Trait,na.rm=FALSE),
             var = var(Trait,na.rm=FALSE),
             sd = sd(Trait,na.rm=FALSE))

stat_l1 <- as.data.frame(mean_l)

#trait count, min, max, mean, sum, median, var, sd over year
...
stat_y1 <- as.data.frame(mean_y)

#trait count, min, max, mean, sum, median, var, sd over genotype
...
stat_g1 <- as.data.frame(mean_g)

#trait count, min, max, mean, sum, median, var, sd over environment
...
lcgcorr <- lgmean %>%
  group_by(LC) %>%
  do(tidy(cor.test(.$glcmean, .$gmean, method = c("pearson"))))
...
""
print(BLUP_CLT1, row.names = FALSE)
""
"########################################################################"
"##         ANOVA Case 2: CLT, YR and LC - Fixed; RP - Random          ##"
"########################################################################"
...
"#trait count, min, max, mean, sum, median, var, sd over location"
""
print(stat_l1, row.names = FALSE)
""
"#trait count, min, max, mean, sum, median, var, sd over year"
""
print(stat_y1, row.names = FALSE)
""
"#trait count, min, max, mean, sum, median, var, sd over genotype"
""
print(stat_g1, row.names = FALSE)
""
"#trait count, min, max, mean, sum, median, var, sd over environment"
""
print(stat_e1, row.names = FALSE)
""
